...
export function AuthProvider({ children }) {
    ...
    const { data: token, isLoading, } = useQuery(['user-me', token], async () => await SecureStore.getItemAsync('token'));

    const { isFetched, refetch } = useQuery(['user-me', token], () => {
        if (token && token !== null) {
